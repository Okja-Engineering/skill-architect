...
|---|---|---|
| Claude Code | ✅ Implemented | OTel export (tokens, tool calls, timing, skill.name activation when present) |
| Cursor | ✅ Implemented | OTel + SQLite fallback |
| Codex | ✅ Implemented | OTel logs + hooks |
| Devin | ✅ Implemented | ATIF export + server API |
...
- It does not run live paired comparisons itself — the profiler captures runtime signals and the comparison engine (`profiler compare`, F04) reports paired differences; scheduling the runs remains the caller's job.
