...
# Audit scripts are vendored copies of skill-audit's — this skill is
# self-contained and must not reach into a sibling bundle (T019).
audit_scripts="$script_dir"
if [[ ! -x "$audit_scripts/audit-report.sh" ]]; then
  echo "vendored audit scripts not found at $audit_scripts" >&2
  exit 3
fi

if [[ -z "$audit_report" || ! -f "$audit_report" ]]; then
  if "$audit_scripts/audit-report.sh" "$target_skill" > "/tmp/draft-rewrite-audit-${skill_name}.json" 2>/dev/null; then
    audit_report="/tmp/draft-rewrite-audit-${skill_name}.json"
  else
    audit_report="$(mktemp)"
    "$audit_scripts/check-frontmatter.sh" "$target_skill" > "$audit_report" 2>&1 || true
    "$audit_scripts/check-structure.sh" "$target_skill" >> "$audit_report" 2>&1 || true
  fi
fi
