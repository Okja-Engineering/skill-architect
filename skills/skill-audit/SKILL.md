...
```bash
command -v skill-validator >/dev/null 2>&1 || echo "skill-validator not found — spec checks will report 'unknown'. Install with: brew install agent-ecosystem/tap/skill-validator" >&2
command -v skillscore >/dev/null 2>&1 || echo "skillscore not found — quality scoring will report 'unknown'. Install with: npm install -g skillscore" >&2
