...
**CI wiring (V4-#7).** `tests/test_spike_activation.sh` is discovered by the `tests/test_*.sh` glob **S1a DoD 7** installs in `.github/workflows/ci.yml`; this slice edits no workflow file. It reads a committed fixture and needs no build, so it satisfies S1a DoD 7 rule (a) trivially. If SP1 lands before S1a — the likely order, since SP1 waits only on User Question 1 — the test is green locally and unwired until S1a merges; say so in the PR.
