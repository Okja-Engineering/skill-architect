...
- `trap 'rm -f "$audit_report"' EXIT` is installed immediately after the
  `mktemp`, two lines above the first child call. **No other `trap` exists in
...
went from 45 to 50 because its per-suite checks are a glob and now cover it.
