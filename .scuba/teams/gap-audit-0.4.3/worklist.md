...
| **G2-02** | P2 | ah F3 | `claude_code.go:221-230`; `otlp.go:428` | A record is **not** Claude Code's if its scope says it is another product's — an exclusion, not an allowlist: a scope that names no library is read, because an instrumentation scope is optional in OTLP and a collector or receiver in the path may drop it, and the `session.id` test still stands over those records. **Restated from "a record is Claude Code's only if its scope says so" by chief-of-staff ruling on PR #7 (D1, deviation ACCEPTED)**: read absolutely, that wording refuses every record from a pipeline that drops the scope, trading a wrong number for no number. Measured at head: 36 unnamed scope entries across 25 of the 59 committed fixtures, and mutating the rule to a strict allowlist fails 53 tests and subtests across 16 top-level tests. Not an unnoticed miss — a deliberate, accepted deviation. | Records from scope `some.other.product` named bare `tool_result` / `api_request` must not appear in `tool_calls`; and a record whose scope names nothing must still be read. |
