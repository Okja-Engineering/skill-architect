...
**Updated** 2026-09-13 22:40 CDT · `scribe` · mirror `scuba-state/imagineux-gmail-com@1be5281` (pushed & SHA-verified)
...
- 🔵 **release/0.4.2 — the next cut, mandated and ready to dispatch** — the accumulator's time-series model. Both undercounts were reproduced against a binary built from `82ecdbb`, and they share **one root**: the adapter keys a series on the data-point attribute set alone, while OTel identifies a series by resource + scope + metric + attributes, and a cumulative series additionally needs `startTimeUnixNano` to detect resets. So it ships as **one PR**, cut from `origin/main` at `2ed34b8`, not from local `main`. No schema change — `profile/v1` keys are unchanged, only values become correct. Delta temporality, which is Claude Code's default, is unaffected. A **sibling PR** for the two skill-audit script guards is mandated alongside it; whether it ships inside 0.4.2 is an open call below. → [mandate](teams/release-0.4.2/mandate.md) · [sibling mandate](teams/release-0.4.2-scripts/mandate.md)
...
- 📋 **Release ladder (decided 2026-09-13)** — user call; plan: [~/.claude/plans/determine-the-next-menaingful-glistening-horizon.md](file:///Users/matthewvandusen/.claude/plans/determine-the-next-menaingful-glistening-horizon.md). Priority order **0.4.1 → 0.4.2 → 0.5.0 → 0.6.0 → 0.7.0 → 1.0**. (Held here rather than as a new `##` section: the roadmap's three-section frame is frozen; the ladder's canonical home is the tree below.)
  ...
  - **0.4.2 · patch — the accumulator's time-series model.** Two reproduced undercounts under cumulative temporality, one root: resource and instrumentation-scope identity are not part of the token series key, and `startTimeUnixNano` is unread so a counter reset discards the earlier run. Delta, which is Claude Code's default, is unaffected. One PR, cut from `origin/main` at `2ed34b8`. A **sibling mandate** for the two skill-audit script guards is now on disk and assumes it ships in the same release from its own branch — that placement is the open call below. → [mandate](teams/release-0.4.2/mandate.md) · [sibling mandate](teams/release-0.4.2-scripts/mandate.md)
  - **0.5.0 · minor — the profiler grows up.** The 7 unpushed commits (Cursor/Codex/Devin adapters, `compare`, `experiment`) — which now need a **rebase onto the merged `main` at `2ed34b8`**, local `main` being 7 ahead / 1 behind — plus the **uncommitted 0.5.0 prototype in the primary tree, which carries forward**: the hook spool, skill-rewrite self-contained, profile schema **v2** (`cache_write` rename, Duration acknowledged), Devin honesty, version lockstep + drift asserts. **Also inherits 0.4.1's found-not-fixed, carried from [status.md](teams/release-0.4.1/status.md):** `check-frontmatter.sh` prints `frontmatter OK` when `skill-validator` is missing (127 falls through to the license gate); `check-structure.sh --json` reports a failing skill as a clean pass when its only faults are path findings and `jq` is absent (the child's 127 is lost too) — **those two have now been pulled forward into a 0.4.2 sibling mandate; the placement is an open call below**; the whole-export slurp memory ceiling (~6× the file's size in RSS); thin `profiler/cmd` coverage (12.6%); unpinned CI installs (`npm install -g skillscore` untagged; CI still runs no `go vet`/`-race`/`gofmt`); `profiler -h`, `probe -h` and `capture -h` exit 1; the not-a-count bound wording at `claude_code.go:343`; the W4 tool-call success rename; the Devin/Codex/Cursor install rows still unverified (README marks all three "not verified in this release"); `CaptureOpts.APIKey` never read.
...
8. **D1–D8 — ratified by default, defaults in force** per `docs/skillgate-intent.md`. No further call needed; they govern 0.6.0's skill-gate v1 scope and gate neither 0.4.2 nor 0.5.0.
9. **E-CS forks (old §E Q1–Q8) + S0 dispatch** — deferred; S0 (`cursor.go` vs wire reference) still wanted, `cursor.go` is dirty in tree — verify before scoping.
...
flowchart TD
  ...
  R041 --> R042[🔵 0.4.2 · the accumulator's time-series model]:::plan
  ...
  click R042 "teams/release-0.4.2/mandate.md" "Mandate — one root, one PR, cut from main at 2ed34b8"
