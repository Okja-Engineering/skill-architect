...
- Removed `CaptureOpts.OtelEndpoint`, and `CapabilityReport.AnySource` with it. No adapter read either.

**Documentation**

- `README.md` documents the Claude Code install route that works — `claude plugin marketplace add` followed by `claude plugin install skill-architect@skill-architect` — verified live against a clean install.
...
- 0.4.0's profiler test count is stated per commit: 14 at 0.4.0 itself, 15 after 30f374c. This release has **22** test functions (20 in the profiler package, 2 in the CLI), 42 cases counting subtests.
