...
- Added Claude Code adapter (Slice 1) — reads OTel export data for token counts, tool calls, and timing. Skill activation is read from `skill.name` on OTel metrics when exported; per-skill output attribution remains `unknown` (no stable convention exists).
