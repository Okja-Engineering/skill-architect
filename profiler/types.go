...
const (
	...
	// payloads (e.g. chars/4) — never a measured or billed count.
...
type TokenCounts struct {
	...
	// CacheWrite is named for the upstream "cache_write" convention (Cursor's
	// Admin API spells it cacheWriteTokens). Sources still emitting
	// "cache_creation" are accepted on input and normalized here.
	CacheWrite int `json:"cache_write,omitempty"`
	Reasoning  int `json:"reasoning,omitempty"`
}

// ToolCallEntry records a single tool invocation. Failure is the presence of
// ErrorType, matching the OTel model; Success is retained for readability.
// Count carries the aggregate when the source is a delta-aggregated metric
// (e.g. Cursor's cursor.tool.calls); absent means one call.
...
// Confidence names how the link was derived — "inferred" for heuristic
// (file-path) attribution, per spec.md's honesty contract.
...
// EstimatedTokens is a chars/4 estimate over hook payload bytes — a relative
// signal only, never billed usage. Lives outside TokenCounts on purpose:
// the MetricResult contract exists so an estimate is never read as a
// measured value (spec fork vi).
type EstimatedTokens struct {
	Total int64 `json:"total"`
}

// EstimatedTokensResult is a typed MetricResult for the estimate field.
...
type Profile struct {
	...
	// EstimatedContextTokens is the separately-labelled estimate field.
	// `tokens` stays unknown unless a real measured source exists.
...
// PresentEstimatedTokensResult creates an EstimatedTokensResult marked present.
...
// UnknownEstimatedTokensResult creates an EstimatedTokensResult marked unknown.
