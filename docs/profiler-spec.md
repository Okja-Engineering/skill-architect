...
- **Two limits of that series key, both fixed together in 0.5.0.** OTel identifies a time series by its resource, its instrumentation scope, its metric and its attributes; this adapter keys on the data-point attributes alone, and it does not read `startTimeUnixNano`. Under delta temporality neither costs anything — the points add up whatever series they belong to — and delta is what Claude Code emits by default (`aggregationTemporality: 1`, observed live). Under cumulative temporality both undercount. **Resource and instrumentation-scope identity are not part of the series key**, so a capture aggregating several resources merges their series into one: two `resourceMetrics` entries whose data points carry identical attributes — a collector fanning in two `service.instance.id`s — report the latest point rather than the sum, and cumulative 100 and 200 give 200 where the capture holds 300. The same pair under delta correctly gives 300. **`startTimeUnixNano` is not read**, and it is what marks a cumulative series' reset boundary, so a counter that restarts inside one capture discards the run before the reset: 100 over `startTimeUnixNano` 1–10 followed by 20 over 11–20 reports 20 where the capture holds 120. Both trace to one cause — a series modelled as its data-point attribute set rather than as resource + scope + metric + attributes, with a start time — which is why they are one change rather than two.
