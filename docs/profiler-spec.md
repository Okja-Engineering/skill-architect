...
12. An export file that parses but carries no `resourceMetrics` and no `resourceLogs` yields `unknown` for all three, naming the format expected — never `error`, and never a silent zero.
