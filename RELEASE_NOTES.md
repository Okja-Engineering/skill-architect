...
- 55 test functions pass — 50 in the profiler package, 5 in the CLI — and 165 subtests, under `-race`. The shell suites are at 134: 0.4.0's 131 plus three new version-surface assertions. CI now runs `gofmt -l`, `go vet` and `go test -race` rather than `go test` alone.
