...
- Claude Code adapter reads OTel export data for token counts (including reasoning tokens), tool calls, timing, and `skill.name` activation when exported. Per-skill output attribution remains `unknown` — no stable convention exists.
