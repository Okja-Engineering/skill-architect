...
- 55 test functions pass — 50 in the profiler package, 5 in the CLI — and 166 subtests, under `-race`. The shell suites are at 134: 0.4.0's 131 plus three more — the two new marketplace-manifest assertions and the adapter-version one. CI now runs `gofmt -l`, `go vet` and `go test -race` rather than `go test` alone.
